...
Надо признать, что идея децентрализации хороша, но только на уровне транспорта (web, bittorrent — яркие примеры). Переносишь её на уровень сервиса, и мы даунгрейдимся в эпоху ADSL-соединения с интерфейсом начала нулевых. Если на это не согласится большинство человечества, это не выход.
...
5. **никаких централизованных персонализированных рекламных SDK.** То есть баннерку от Google уже на сайт не встроишь. Это существенно ограничивает рекламную модель, но и тут инновации возможны (децентрализованная модель? баннерка, ограничивающая сбор данных?)
