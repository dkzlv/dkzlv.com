<script>
  ...
  let counter = 0;
  $: confessionClasses =
    counter >= 1 ? "confession" : "confession confession--hidden";
  $: counter === 2 && (confessionEl.innerText = "всё еще пидор");
  $: counter === 3 && (confessionEl.innerText = "хорош уже");
  $: counter === 5 && (confessionEl.innerText = ":(");
  $: counter === 10 && (confessionEl.innerText = "ПАСХАЛКА");
  $: counter === 28 && (confessionEl.innerText = "ПАСХАЛКА №2");
  $: counter === 50 && (confessionEl.innerText = "А ты упорный, молодец");
